...
<template>
	<div id="productGridContainer" class="w-full flex items-center justify-center">
		<div class="w-full lg:w-4/5 px-8 py-10 flex flex-col lg:flex-row items-start justify-center">
			...
			<aside ref="aside"
				class="hidden lg:block w-full h-full sm:w-1/2 md:w-1/3 sm:h-full lg:w-[22.5%] lg:h-auto px-8 py-10 lg:px-3.5 lg:py-2.5 text-left fixed lg:sticky left-0 right-0 bottom-0 lg:top-0 sm:border-r-[3px] lg:border-none border-orange-200 dark:border-stone-700 bg-orange-100 dark:bg-stone-800 lg:bg-transparent">
			...
			<section class="w-full lg:w-[77.5%] px-3.5 py-2.5">
				...
				<div
					...
					<a v-for="product of props.catalogue.products"
						...
						<div
							class="relative w-full aspect-square flex items-center justify-center bg-orange-300 dark:bg-stone-900 rounded-l-lg xs:rounded-none xs:rounded-t-lg">
							<svg v-if="!product.media" xmlns="http://www.w3.org/2000/svg" width="25%" height="25%"
						...
						<div
							class="grow w-full h-full xs:h-auto px-3.5 py-2.5 flex flex-col items-start justify-center rounded-r-lg xs:rounded-none xs:rounded-b-lg group-hover:bg-red-300 dark:group-hover:bg-red-400">
